...
export default function LoginPage() {
  ...
  if (step === "2fa") {
    ...
    return (
      <Reveal>
        <AuthCard title={t2fa("title")} description={t2fa("subtitle")}>
          <form onSubmit={handle2faSubmit} className="space-y-5">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                maxLength={6}
                className={cn(authInputClass, "text-center text-lg tracking-[0.4em]")}
                value={code}
            ...
            <Button type="submit" className={authSubmitButtonClass} disabled={verify2faMutation.isPending || secondsLeft <= 0}>
              {verify2faMutation.isPending ? t2fa("submitting") : t2fa("submit")}
              {!verify2faMutation.isPending && <ArrowRight className="size-4" />}
            </Button>
      ...
      <AuthCard title={t("title")} description={t("subtitle")}>
        <form onSubmit={handleCredentialsSubmit} className="space-y-5">
          ...
          <div className="space-y-2">
            ...
            <PasswordInput id="password" autoComplete="current-password" className={authInputClass} value={password} onChange={(e) => setPassword(e.target.value)} required />
          </div>
          <Button type="submit" className={authSubmitButtonClass} disabled={loginMutation.isPending}>
